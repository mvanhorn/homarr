...
Storage monitoring supports array disks and storage pools, including pool-only systems. Pool usage uses the filesystem capacity reported by Unraid. In multi-device pools, secondary members without filesystem capacity do not appear as separate capacity entries; their health status and reported temperatures remain available.
